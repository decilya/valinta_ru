<?php
/** @var array $ordersCustomerArrForEmail */
?>
<style>
    .mainBlock {
        width: 100%;
...
<div class="mainBlock">
    ...
    <h2>Уважаемый(ая) <?= $ordersCustomerArrForEmail['customer']->name ?>!</h2>

    <p class="text">Вы получили новые отклики на следующие заказы: </p>

    <?php foreach ($ordersCustomerArrForEmail['article'] as $article) { ?>

        <article>
            <header><b><?= $article['orderTitle'] ?></b></header>
            <ul>
                <?php
                foreach ($article['users'] as $user) { ?>
                    <li><?= ($user['user']->fio); ?>,
                        соответствие заказу - <?= $user['percentUserByOrder']; ?>%,
                        <a href="<?= Yii::$app->params['host'] . '://' .
                        Yii::$app->params['domen'] .
                        '/customer/customer-order-feedback/' .
                        $article['order']->id .
                        '?user=' . $user['user']->id
                        ?>">перейти к анкете
                        </a>
