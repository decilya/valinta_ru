...
<div class="mainBlock">
    ...
    <p>
        С радостью сообщаем, что на портале «Valinta.ru» размещён заказ, который может вас заинтересовать.<br><br>
        Посмотреть информацию о заказе №<?= $order->id; ?> от <?= date('d-m-Y', $order->updated_at); ?> вы можете,
        перейдя по ссылке:

        <?php
        $protocol = Yii::$app->params['host'] . '://';
        $host = Yii::$app->params['domen'] . "/";
