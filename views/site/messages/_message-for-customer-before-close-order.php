...
<div class="mainBlock">
    ...
    <h2>Уважаемый(ая) <?= $order->fio ?>!</h2>

    <p>Сообщаем, что Ваш заказ №<?= $order->id; ?> от <?= date('d-m-Y', $order->updated_at) ?> будет закрыт через <?= Yii::$app->params['dayToSendMailAboutFinishOrder']; ?> календарных дней.</p>
