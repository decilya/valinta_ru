<?php

use yii\helpers\Url;

/**
 * @var \app\models\Order $order
 */
?>

<style>
/*    .mainBlock {
        width: 100%;
    }

    .mainBlock .header {
        height: 60px;
        width: 100%;
    }

    .mainBlock .header img {
        display: block;
        float: left;
        margin-top: 15px;
        margin-left: 30px;
    }

    .mainBlock .header .contactsBlock {
        display: block;
        float: right;
        margin-right: 30px;
        margin-top: 15px;
    }

    .mainBlock .header .contactsBlock p {
        padding: 0;
        margin: 0;
    }

    .mainBlock .header .contactsBlock p:first-child {
        padding-bottom: 5px;
    }

    .mainBlock h2 {
        text-align: center;
        background-color: #1083ab;
        color: #ffffff;
        padding: 10px 0;
    }

    .mainBlock ul {
        list-style-type: none;
    }

    .mainBlock .text {
        padding: 20px 0 30px 30px;
    }

    .mainBlock .parameters {
        color: #ffffff;
        font-weight: bold;
        font-size: 150%;
        display: inline-block;
        border-top-right-radius: 10px;
        border-bottom-right-radius: 10px;
        padding: 10px 30px 10px 30px;
        background-color: #1083ab;
    }

    .mainBlock .regards {
        font-style: italic;
        text-align: right;
        padding-right: 30px;
    }*/
</style>
<div class="mainBlock">
    <div class="header" style="height: 60px;width: 100%;">
        <img style="display: block;float: left;margin-top: 15px;margin-left: 30px;" src="data:image/png;base64,"/>

        <div class="contactsBlock" style="display: block;float: right;margin-right: 30px;margin-top: 15px;font-family: Arial, sans-serif;font-size: 15px;padding: 0;">
            <p style="padding-bottom: 5px;margin: 0;">Телефон: <?= Yii::$app->params['mainSystemPhone']; ?></p>

            <p style="padding: 0;margin: 0;">
                E-mail: <a href="mailto:<?= Yii::$app->params['mailFrom']; ?>"><?= Yii::$app->params['mailFrom']; ?></a>
            </p>
        </div>
    </div>
    <h2 style="font-family: Arial, sans-serif; font-weight: 400;background-color: #1083ab;padding: 10px 0;color: #FFF; text-align: center;">Уважаемый(ая) <?= $order->fio ?>!</h2>

    <p style='font-family: "Arial", sans-serif;'>С радостью сообщаем, что Вы успешно завершили регистрацию заказа на портале «Valinta.ru». Ваш заказ
        №<?= $order->id; ?> от <?= date('d.m.Y', time()); ?> размещён на нашем
        портале сроком на <?= Yii::$app->params['dayToFinishNewOrder']; ?> календарных дней.
        Информация о заказе разослана всем подходящим сметчикам, зарегистрированным на портале.<br><br>

        Внести необходимые изменения в заказ вы всегда можете на странице редактирования, перейдя по ссылке:

        <?php
        $protocol = Yii::$app->params['host'] . '://';
        $host = $_SERVER['HTTP_HOST'];
        $url = Url::to(['order/update-customer-order', 'id' => $order->id]);

        $link = $protocol . $host . $url; ?>

        <a href='<?= $link ?>' target='_blank'><?= $link; ?></a><br><br>

        В случае возникновения каких-либо вопросов, пожалуйста, свяжитесь с нами по
        телефону:<?= Yii::$app->params['mainSystemPhone']; ?><br><br>

    </p>

    <p style="font-family: Arial, sans-serif;font-size: 16px;font-style: italic;text-align: right;">С уважением,<br>
        команда Valinta.ru</p>

        <div style='font-size: 14px; text-align: center;'>

    <p style="font-family: Arial, sans-serif;font-size: 14px;text-align: center;">Данное письмо создано и отправлено автоматически, не нужно на него
        отвечать.</p>
</div>
</div>