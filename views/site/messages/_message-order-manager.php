...
<p>На портале «Valinta.ru» размещён новый заказ.</p>
...
<table cellpadding="5">
    ...
    <tr>
        <td>Телефон:</td>
        <td><?= $order->phone ?><?php if(!empty($order->extraPhones)) foreach($order->extraPhones as $phone) if(!empty($phone)) echo ", ".$phone; ?></td>
    </tr>
    ...
    <?php if (!empty($order->professions)) : ?>
